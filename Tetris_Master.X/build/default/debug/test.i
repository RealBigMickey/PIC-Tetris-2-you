...
void update_display(void) {
    ...
    for (int row_addr = 0; row_addr < 8; row_addr++) {


        uint8_t pattern_bot = 0;
        for (int col = 0; col < 8; col++) {
            if (board[row_addr][col]) {
                pattern_bot |= (1 << col);
            }
        }
...
void main(void) {
    ...
    TRISBbits.RB0 = 0b1;
    INTCONbits.INT0IF = 0b0; INTCONbits.INT0IE = 0b1;

    serialPrint("\x1B[2J\x1B[H\x1B[3J");
    ...
    while(1) {
        update_display();
        uint8_t temp_row[8];
        memcpy(temp_row, board[15], sizeof(board[15]));
        memmove(&board[1][0], &board[0][0], sizeof(board[0]) * 15);
        memcpy(board[0], temp_row, sizeof(board[0]));
